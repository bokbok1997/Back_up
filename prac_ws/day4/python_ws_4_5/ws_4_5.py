...
def create_user(onelist):
    wrong_cnt=0
    user_list=[]
    for len_cnt in onelist:
        tf = is_validation(len_cnt)

        if tf == True:
            user_list.append(len_cnt)
        elif tf == 'blocked':
            wrong_cnt+=1
        else:
            wrong_cnt+=1
            for i in tf[1]:
                len_cnt[i]= None
            user_list.append(len_cnt)
    print(f'잘못된 데이터로 구성돤 유저의 수는 {wrong_cnt}입니다')
    return user_list

def is_validation(data1):
    temp=[]
    cnt=0
    if data1['blood_group'] not in blood_types:
        temp.append('blood_group')

    if data1['company'] in black_list:
        temp.append('company')
        return 'blocked'
    if '@'not in data1['mail']:
        temp.append('mail')

    if (2>len(data1['name']))or(30<len(data1['name'])):
        temp.append('name')

    if len(data1['website'])<1:
        temp.append('website')
